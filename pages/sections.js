...
// ── Vertical nav sections (hidden — used for active-class tracking only) ──────
const NAV_ITEMS = [
  { id: "home",      label: "Home" },
...
export default function SectionsPage() {
  ...
  useEffect(() => {
    // On mount: scroll to hash target
    ...
    <>
      ...
      <Head>
        ...
        <link
          rel="preload"
          as="image"
          href="/img/lsk_imgs/business_home.png"
          type="image/png"
        />
      </Head>
      ...
      {/* ── Vertical nav (accessibility / SEO hidden — active-class tracker) ── */}
      ...
      <Footer
        ...
        content={
          ...
          >
            ...
            >
              © {new Date().getFullYear()} Aurora Blossom Tech. All rights reserved.
            </Typography>
            <Box sx={{ display: "flex", gap: 3 }}>
              {["Home", "Services", "Careers", "Contact"].map((label) => (
                <Box
                  key={label}
                  component="a"
                  href={`/sections#${label.toLowerCase().replace(" ", "")}`}
                  sx={{
                    fontFamily: '"Montserrat", sans-serif',
                    fontWeight: 600,
                    fontSize: "0.75rem",
